import {PETS,petSelection} from '/67park-foundation-next/app/pets/pet-state.js?v=pets-soft-2';
  ...
  return function PetInventory(){
    ...
    return h('section',{className:'park-pet-inventory','aria-label':'Pet companions'},
      ...
      h('div',{className:'park-pet-choices'},PETS.map(p=>h('button',{key:p.id,type:'button','aria-label':'Choose '+p.label,'aria-pressed':selected===p.id,onClick:()=>petSelection.select(p.id),style:{'--pet-accent':p.accent}},
        h('img',{src:new URL('/67park-foundation-next/pets/'+p.id+'.png?v=pets-soft-2',import.meta.url).href,alt:'',width:92,height:85}),h('strong',null,p.name),h('small',null,selected===p.id?'Following you':p.label)))),
      h('div',{className:'park-pet-tools'},h('button',{type:'button',disabled:!selected,onClick:()=>petSelection.select('')},'No pet'),h('a',{href:new URL('/67park-foundation-next/pets/?v=pets-soft-2',import.meta.url).href,target:'_blank',rel:'noopener'},'View in 3D ↗')),

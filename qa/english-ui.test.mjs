...
test('English revision invalidates the changed entry and loader without replacing game singleton revisions',()=>{
 assert(read('index.html').includes('app/main.js?v=camera-touch-1"'));
 assert(read('explore/index.html').includes('explore.js?v=photo-surfaces-1"'));
 for(const file of ['app/main.js','explore/explore.js'])assert.match(read(file),/runtime\.bundle\.js\?v=photo-surfaces-1["']/);

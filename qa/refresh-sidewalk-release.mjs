...
for(const file of ['island/runtime.js','island/runtime.bundle.js']){
 ...
 const marker=/(renderer|r)\.domElement\.dataset\.mapEdgeFinish1=JSON\.stringify\(applyMapEdgeFinish\((kok|h0),await (islandFetch|__boundaryFetch)\('\/67park-foundation-next\/repairs\/map-edge-finish-1.json'\)\.then\(r=>\{if\(!r.ok\)throw Error\('Map edge repair missing'\);return r.json\(\)\}\)\)\);/g;

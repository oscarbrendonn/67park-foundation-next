...
for(const file of ['island/runtime.js','island/runtime.bundle.js']){
 ...
 const statement=`${r}.domElement.dataset.northHousingSurface1=JSON.stringify(applyNorthHousingSurface(${root},await ${fetch}('/67park-foundation-next/repairs/north-housing-surface-1.json?v=north-housing-5').then(r=>{if(!r.ok)throw Error('Northern housing repair missing');return r.json()})));`;
 text="import {applyNorthHousingSurface} from '../app/north-housing-surface.js?v=north-housing-5';\n"+text.slice(0,start)+statement+text.slice(start);
...
for(const file of ['app/main.js','explore/explore.js','index.html','explore/index.html']){
 ...
 const next=old.replace(/(runtime\.bundle\.js|app\/main\.js|explore\.js)\?v=(?:gorilla-only-1|park-animals-solid-1)/g,'$1?v=north-housing-5');

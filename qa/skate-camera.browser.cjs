...
module.exports=async(page,{mobile,check})=>{
 ...
 await check('real board jump route keeps city camera collision bounded',async()=>{
  ...
  try{
   ...
   await page.keyboard.down('KeyD');await page.waitForTimeout(800);await page.keyboard.up('KeyD');await page.waitForTimeout(80);
  ...
  if(!process.env.PARK_SOFTWARE_RENDER){assert(result.rendered>=20,JSON.stringify(result));assert(result.gaps.p95<50,JSON.stringify(result));assert(result.gaps.max<250,JSON.stringify(result));}

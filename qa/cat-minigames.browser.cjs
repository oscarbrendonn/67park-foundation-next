// Narrow Cat selection/animation entry check, not a full game regression.
const {chromium}=require('playwright'),fs=require('node:fs'),assert=require('node:assert/strict');
const {browserLaunchOptions}=require('./browser-launch.cjs');
const base=process.env.PARK_CAT_URL||'http://127.0.0.1:8496/67park-foundation-next/';
const out=process.env.PARK_CAT_EVIDENCE||'.qa-results/cat-minigames-1';
...
(async()=>{
 ...
 try{for(const game of ['lane-rush','skybound-soft']){
  ...
  page.on('pageerror',e=>errors.push(e.message));page.on('request',r=>{if(r.url().includes('cat-gorilla-body.glb'))requests.push(r.url())});
  await page.addInitScript(()=>{localStorage.setItem('67park-feel-lab-muted','1');localStorage.setItem('67park-feel-lab.character.v3',JSON.stringify({base:'cat67',body:'friendsie_2:2',head:'friendsie_26:90',sprout:'friendsie_8:90',back:'friendsie_2:4',kicks:'friendsie_3333:5',held:null,power:null,vibe:null}));});
  try{
   ...
   const initial=await read(page);assert.equal(game==='lane-rush'?initial.racers[0].base:initial.base,'cat67');assert(requests.length>0);
   ...
   assert.equal(game==='lane-rush'?moving.racers[0].base:moving.base,'cat67');assert.deepEqual(errors,[]);
   report.games.push({game,initial,moving,requests,errors});console.log('CAT_MINIGAME_PASS',game);

 ...
 try{for(const mobile of [false,true]){
  ...
  try{const page=await context.newPage(),errors=[];
   ...
   const report=await page.evaluate(async({cornerOnly,westOnly})=>{
    ...
    for(const x of [-15.102,-15.11,-15.15,-15.3])for(const z of [-240.870,-240.85,-240.80,-240.754,-240.75,-240.7])points.push({kind:'paving',x,z});
    ...
    for(let i=0;i<position.count;i++){v.fromBufferAttribute(position,i).applyMatrix4(roadMesh.matrixWorld);if(v.x> -15.11&&v.x< -4.52&&v.z< -230&&v.y>9.2)roadFront=Math.min(roadFront,v.z)}
    const poolFront=JSON.parse(w.renderer.domElement.dataset.northPool104).parcel.north;
   ...
   assert(report.alignment.difference<.00002,'Road and pool parcel fronts must align: '+JSON.stringify(report.alignment));
